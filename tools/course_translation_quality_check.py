...
bad_mix=re.compile(r'[\u4e00-\u9fff]\s+[A-Za-z]|[A-Za-z]\s+[\u4e00-\u9fff]')
for w in words:
    z=(w.get('chinese') or '').strip()
    if not z: err(f'{w.get("id")} {w.get("word")}: empty Chinese course meaning')
    if bad_en.search(z) or bad_mix.search(z): err(f'{w.get("id")} {w.get("word")}: machine-fragment Chinese remains: {z!r}')
