...
for i,u in enumerate(units):
    ...
    # v3.3.4: every pre-challenge node has a distinct skill role. New words are distributed
    # across all of them, so a unit no longer spends several consecutive nodes on vocabulary.
    learn=max(1,lc-1);n=len(ids)
    chunks=[]
    covered=[]
    ...
    if chunks and max(chunks)>9: err(f'{uid}: mixed-skill chunk {max(chunks)} exceeds 9-word safe budget: {chunks}')
...
if errors:
    ...
    sys.exit(1)
print(f'Course check OK: 98 units, 2774 words, stages 8/24/30/36, largest unit {max_unit[0]} words ({max_unit[1]}), largest mixed-skill chunk {max_chunk[0]} words ({max_chunk[1]})')
