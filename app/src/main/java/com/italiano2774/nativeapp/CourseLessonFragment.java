...
public class CourseLessonFragment extends Fragment {
    ...
    private List<CourseQuestion> questions=new ArrayList<>();private int lesson,index,correct,wrong;private boolean checked=false,finished=false;private String selectedOption="";private long shownAt;
    ...
    private void onAction(){if(finished){closeToPath();return;}CourseQuestion q=current();if(q==null){showFinish();return;}if(q.type==CourseQuestion.INTRO&&!checked){index++;saveResume();render();return;}if(!checked){evaluate(q);return;}index++;saveResume();render();}
    private void evaluate(CourseQuestion q){String actual=isChoice(q)?selectedOption:(input.getText()==null?"":input.getText().toString());boolean ok=equalsAnswer(actual,q.answer);checked=true;if(ok)correct++;else wrong++;long ms=Math.max(0,SystemClock.elapsedRealtime()-shownAt);if(q.type!=CourseQuestion.INTRO&&q.word!=null){if(q.type==CourseQuestion.ACTIVE)progress.recordDimensionResults(q.word.id,new int[]{ProgressStore.DIM_MEANING,ProgressStore.DIM_SPELLING},ok,ms);else progress.recordDimensionResult(q.word.id,q.dimension,ok,ms);if(!ok)progress.recordErrorCause(q.type==CourseQuestion.LISTEN?ErrorCause.LISTENING_CONFUSION:(q.type==CourseQuestion.SPELL_HINT||q.type==CourseQuestion.ACTIVE?ErrorCause.SPELLING:ErrorCause.RECALL),q.word.id,"course_v3",q.answer,actual,"unit="+unit.id+",lesson="+lesson);}
    ...
    private boolean isChoice(CourseQuestion q){return q.type==CourseQuestion.MEANING||q.type==CourseQuestion.LISTEN||q.type==CourseQuestion.CLOZE||q.type==CourseQuestion.EXAMPLE_MEANING;}
