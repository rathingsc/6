...
public class ProgressStore {
    ...
    public void recordDimensionResults(int id,int[] dims,boolean correct,long responseMs){
        ...
        recomputeOverall(id);updateAdaptiveSchedule(id,correct,responseMs);if(!correct)p.edit().remove("grad_"+id).apply();recordDailyPractice(correct,responseMs);
    ...
    public double memoryStability(int id){double v=p.getFloat("fs_s_"+id,0f);return Double.isFinite(v)?Math.max(0.0,Math.min(3650.0,v)):0.0;}
    public double memoryDifficulty(int id){double v=p.getFloat("fs_d_"+id,0f);return Double.isFinite(v)?Math.max(0.0,Math.min(10.0,v)):0.0;}
    ...
    public void recordStudyRating(int id,int rating){
        ...
        boolean correct=rating>=2;long ms=rating==3?2200:(rating==2?4500:10000);updateAdaptiveSchedule(id,correct,ms);if(!correct)p.edit().remove("grad_"+id).apply();recordDailyCard();LearningDatabase.log(context,"study_rating",String.valueOf(id),DIM_MEANING,correct,ms,"rating="+rating);mirrorWordState(id);
    }

    /** v3.1 three-choice smart vocabulary rating: forgot / fuzzy / remembered. */
    public void recordSmartWordRating(int id,int rating,long responseMs){
        rating=clamp(rating,0,2);int cur=meaningLevel(id),target=SmartMemoryScheduler.nextMeaningLevel(rating,cur);
        SharedPreferences.Editor base=p.edit().putInt(dimKey(id,DIM_MEANING),target);
        for(int dim=1;dim<4;dim++)if(!p.contains(dimKey(id,dim)))base.putInt(dimKey(id,dim),0);base.apply();recomputeOverall(id);if(mastery(id)==0)p.edit().putInt("m_"+id,1).apply();
        int oldStreak=correctStreak(id),nextInterval=SmartMemoryScheduler.nextInterval(rating,oldStreak,cur,intervalDays(id));
        int att=attempts(id)+1,cor=correctAnswers(id)+(rating==2?1:0);long oldAvg=avgResponseMs(id);long sample=responseMs>0?responseMs:(rating==2?3500:(rating==1?7000:10000));long avg=oldAvg<=0?sample:Math.round(oldAvg*0.75+sample*0.25);
        double oldDifficulty=memoryDifficulty(id)>0?memoryDifficulty(id):5.5;double nextDifficulty=Math.max(1.0,Math.min(10.0,oldDifficulty+(rating==0?0.8:(rating==1?0.25:-0.35))));LocalDate today=LocalDate.now();
        SharedPreferences.Editor e=p.edit().putInt("att_"+id,att).putInt("cor_"+id,cor).putLong("last_"+id,today.toEpochDay()).putLong("avgms_"+id,avg).putInt("iv_"+id,nextInterval).putLong("due_"+id,today.plusDays(nextInterval).toEpochDay()).putFloat("fs_s_"+id,(float)Math.max(0.45,nextInterval)).putFloat("fs_d_"+id,(float)nextDifficulty);
        ...
        recordDailyCard();LearningDatabase.log(context,"smart_memory",String.valueOf(id),DIM_MEANING,rating==2,sample,"rating="+rating+",interval="+nextInterval);mirrorWordState(id);
    }

    // ---------- Scheduling + metrics ----------
    public int intervalDays(int id){return clamp(p.getInt("iv_"+id,mastery(id)>0?defaultInterval(mastery(id)):1),1,3650);}
    ...
    public DashboardStats dashboardStats(List<Word> words,LocalDate date){
        ...
        for(int i=0;i<words.size();i++){
            ...
            if(m>0){s.totalIntroduced++;s.learned++;for(int d=0;d<4;d++)learnedSum[d]+=dims[d];long due=dueEpochDay(id);if(due==Long.MIN_VALUE||due<=day)s.due++;}
        ...
        if(s.routeUnknown==0)s.newQuota=0;else{
            int sm=sessionMinutes();int base=sm<=10?5:(sm<=20?10:(sm<=30?15:(sm<=45?20:25)));
            if((s.sevenDayAttempts>=12&&s.sevenDayAccuracy<65)||s.due>=90||s.stubborn>=45)s.newQuota=0;else{double factor=s.sevenDayAccuracy<70?0.45:(s.sevenDayAccuracy<80?0.70:(s.sevenDayAccuracy>=92?1.18:1.0));if(s.due>60)factor*=0.50;else if(s.due>35)factor*=0.70;else if(s.due>20)factor*=0.86;if(s.wrong>25)factor*=0.80;if(s.stubborn>20)factor*=0.82;int q=(int)Math.round(base*factor);s.newQuota=Math.min(s.routeUnknown,Math.max(2,Math.min(sessionMinutes()>=60?30:25,q)));}
        }
        ...
        int smCap=sessionMinutes();int baseCap=smCap<=10?18:(smCap<=20?30:(smCap<=30?48:(smCap<=45?60:72)));s.reviewCap=s.reviewPressure>=3?Math.min(baseCap,42):(s.reviewPressure==2?Math.min(baseCap,48):baseCap);
        if(s.newQuota==0)s.paceAdvice="暂停新词 · 先清理复习压力";else if(s.due>35)s.paceAdvice="复习优先 · 新词 "+s.newQuota;else if(s.stubborn>20)s.paceAdvice="顽固词偏多 · 新词 "+s.newQuota;else if(s.sevenDayAccuracy>=92)s.paceAdvice="状态稳定 · 新词 "+s.newQuota;else s.paceAdvice="平衡推进 · 新词 "+s.newQuota;
    ...
    public int recommendedNewWords(List<Word> words){
        int routeLimit=Math.min(vocabularyRouteLimit(),words.size()),unknown=0;for(int i=0;i<routeLimit;i++)if(mastery(words.get(i).id)==0)unknown++;if(unknown==0)return 0;
        int sm=sessionMinutes();int base=sm<=10?5:(sm<=20?10:(sm<=30?15:(sm<=45?20:25)));int acc=sevenDayAccuracy(),due=dueCount(words,LocalDate.now()),wrong=wrongTotal(words),stubborn=stubbornCount(words);
        if((sevenDayAttempts()>=12&&acc<65)||due>=90||stubborn>=45)return 0;
        double factor=acc<70?0.45:(acc<80?0.70:(acc>=92?1.18:1.0));if(due>60)factor*=0.50;else if(due>35)factor*=0.70;else if(due>20)factor*=0.86;if(wrong>25)factor*=0.80;if(stubborn>20)factor*=0.82;
        int q=(int)Math.round(base*factor);return Math.min(unknown,Math.max(2,Math.min(sessionMinutes()>=60?30:25,q)));
    }
    ...
    public void recordErrorCause(String cause,int wordId,String mode,String expected,String actual,String detail){String c=cause==null?ErrorCause.RECALL:cause;p.edit().putInt(errorKey(c),errorCauseCount(c)+1).apply();LearningDatabase.logError(context,wordId,mode,c,expected,actual,detail);}
    ...
    public JSONObject exportJson(List<Word> words) throws Exception {
        JSONObject o=new JSONObject();o.put("version",18);o.put("startDate",startDate().toString());o.put("perDay",perDay());o.put("preferOriginalAudio",preferOriginalAudio());
        ...
        JSONArray m=new JSONArray(),f=new JSONArray(),wj=new JSONArray(),due=new JSONArray(),metrics=new JSONArray(),dims=new JSONArray(),conf=new JSONArray(),fsrs=new JSONArray(),graduated=new JSONArray();
